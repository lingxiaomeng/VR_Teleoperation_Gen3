...
public class ArmController : MonoBehaviour
{
    ...
    private string mode_control_topic_name = "gen3_mode_control";
    void Start()
    {
        m_ROS = ROSConnection.GetOrCreateInstance();
        m_ROS.RegisterPublisher<PoseMsg>(follow_topic_name);
        m_ROS.RegisterPublisher<Float32Msg>(grasp_topic_name);
        m_ROS.RegisterPublisher<Int8Msg>(mode_control_topic_name);
    ...
    void Update()
    {
        if (follow == false && tracking == true)
        {
            ...
            Int8Msg mode = new Int8Msg();
            mode.data = 1;
            m_ROS.Publish(mode_control_topic_name, mode);
    ...
    private void OnDisable()
    {
        ...
        pose.position.z = 0;
        m_ROS.Publish(follow_topic_name, pose);
        Debug.Log("Stop Tracking Disabled\n");
    }
}
